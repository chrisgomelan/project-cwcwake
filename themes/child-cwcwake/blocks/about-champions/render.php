...
$wrapper = get_block_wrapper_attributes( array( 'class' => 'cwc-champions' ) );
?>

<section <?php echo $wrapper; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> data-cwc-champions>
	...
	<div class="cwc-champions__carousel" data-cwc-carousel>
		<div class="cwc-champions__track">
			<?php foreach ( $images as $idx => $src ) : ?>
				<div class="cwc-champions__slide" data-index="<?php echo esc_attr( (string) $idx ); ?>">
					<img src="<?php echo esc_url( $src ); ?>" alt="CWC Champion <?php echo esc_attr( (string) ( $idx + 1 ) ); ?>" loading="lazy">
				</div>
			<?php endforeach; ?>
		</div>
	</div>
