( () => {
	...
	document.addEventListener( 'DOMContentLoaded', () => {
		...
		guestsModal.addEventListener( 'click', e => e.stopPropagation() );

		// Close button binding
		document.querySelectorAll( '.cwc-ri-modal__close' ).forEach( btn => {
		...
		document.querySelectorAll( '.cwc-room-info__pricing-field[data-modal-target]' ).forEach( trigger => {
			trigger.addEventListener( 'click', e => {
				...
				if ( target === 'date' ) {
					renderCalendar();
					openModal( dateModal, trigger );
				} else if ( target === 'guests' ) {
					openModal( guestsModal, trigger );
				}
			} );
		} );
