...
if ( ! function_exists( 'cwc_render_blog_pagination' ) ) :
	...
	function cwc_render_blog_pagination( int $total_pages, int $current_page, string $category = '' ): string {
		...
		<nav class="cwc-all-blogs__pagination" aria-label="<?php esc_attr_e( 'Blog pagination', 'child-cwcwake' ); ?>">
			...
			<?php else : ?>
				<span class="cwc-all-blogs__page cwc-all-blogs__page--prev cwc-all-blogs__page--disabled" aria-hidden="true">‹</span>
			<?php endif; ?>
			...
			<?php else : ?>
				<span class="cwc-all-blogs__page cwc-all-blogs__page--next cwc-all-blogs__page--disabled" aria-hidden="true">›</span>
			<?php endif; ?>
		</nav>
		<?php
...
<section <?php echo $wrapper_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
	<header class="cwc-all-blogs__header">
		...
		<form class="cwc-all-blogs__filter" method="get" action="<?php echo esc_url( $base_url ); ?>"
			role="search" aria-label="<?php esc_attr_e( 'Filter blog posts by category', 'child-cwcwake' ); ?>">
			<label class="cwc-all-blogs__filter-label" for="cwc-all-blogs-filter">
				<?php esc_html_e( 'Filter:', 'child-cwcwake' ); ?>
			</label>
			<select id="cwc-all-blogs-filter" name="blog_cat" class="cwc-all-blogs__filter-select">
			...
			<noscript>
				<button type="submit" class="cwc-all-blogs__filter-submit">
					<?php esc_html_e( 'Apply', 'child-cwcwake' ); ?>
				</button>
			</noscript>
		</form>
	</header>
	...
	<?php if ( empty( $posts ) ) : ?>
		<p class="cwc-all-blogs__empty"><?php esc_html_e( 'No blog posts found for this filter.', 'child-cwcwake' ); ?></p>
	<?php else : ?>
			...
			foreach ( $posts as $post_obj ) :
				...
				<article class="cwc-all-blogs__card">
					...
					<div class="cwc-all-blogs__card-body">
						...
						<?php if ( '' !== $date ) : ?>
							<time class="cwc-all-blogs__card-date" datetime="<?php echo esc_attr( get_the_date( DATE_W3C, $post_id ) ); ?>">
