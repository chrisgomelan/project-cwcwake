...
(() => {
	document.addEventListener('DOMContentLoaded', () => {
		...
		const runAvailabilityCheck = async () => {
			...
			const ciStr = formatLocalDate(selectedStart);
			const coStr = formatLocalDate(selectedEnd);
		...
		if (bookingData && bookingData.room) {
			fetchBookedDates(bookingData.room);
		}
		...
		const updateModalPills = () => {
			...
			if (valCheckin) valCheckin.textContent = selectedStart ? formatDate(selectedStart) : 'Select Date';
			if (valCheckout) valCheckout.textContent = selectedEnd ? formatDate(selectedEnd) : 'Select Date';
		};
			...
			btn.addEventListener('click', () => {
				...
				if (modal.id === 'bf-modal-trip-summary') {
					...
					if (selectedEnd && summaryCheckout) {
						const dateStr = formatDate(selectedEnd);
		...
		const syncInitialUI = () => {
			...
			if (summaryRoomName) {
				...
				if (roomRadio) {
					...
					if (roomRadio.dataset.capacity) {
						maxCapacity = parseInt(roomRadio.dataset.capacity, 10) || 4;
					}
				}
			}
